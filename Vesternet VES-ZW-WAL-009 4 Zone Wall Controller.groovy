...
metadata {
	definition (name: "Vesternet VES-ZW-WAL-009 4 Zone Wall Controller", namespace: "Vesternet", author: "Vesternet", mcdSync: true, ocfDeviceType: "x.com.st.d.remotecontroller", mnmn: "Sunricher", vid: "generic-8-button") {
		capability "Button"
		capability "Sensor"
		capability "Battery"

		fingerprint mfr: "0330", prod: "0300", model: "A305", deviceJoinName: "Vesternet VES-ZW-WAL-009 4 Zone Wall Controller"		
	}
    preferences {		
...
def addChildButtons(numberOfButtons) {
	...
	for (def endpoint : 1..numberOfButtons) {
		try {
			...
			def child = addChildDevice("Vesternet", "Vesternet VES-ZW-WAL-009 4 Zone Wall Controller Child Button", childDni, device.getHub().getId(), [
					completedSetup: true,
